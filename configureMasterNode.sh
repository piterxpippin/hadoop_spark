...
ssh -o StrictHostKeyChecking=no -i $keyPair ec2-user@$nodeAddress 'cd database; for file in $(curl http://ftp.sunet.se/mirror/archive/ftp.sunet.se/pub/tv+movies/imdb/ | grep --color=never -Po "\>(\K[a-z\.-]+\.gz)"); do wget http://ftp.sunet.se/mirror/archive/ftp.sunet.se/pub/tv+movies/imdb/$file & done'
ssh -o StrictHostKeyChecking=no -i $keyPair ec2-user@$nodeAddress 'cd database; for file in $(ls); do gunzip $file & done'
