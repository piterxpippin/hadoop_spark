...
sleep 10
ssh -o StrictHostKeyChecking=no -i $keyPath ec2-user@$namenodeAddress 'screen -S kafkaRun -d -m sudo -S -u hduser -i /bin/bash -l -c "bin/kafka-server-start.sh $KAFKA_HOME/config/server.properties"'
sleep 10
